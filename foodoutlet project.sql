...
-- 1. Find top 3 outlets by cuisine type without using limt and top function
...
select * 
from (select *, ROW_NUMBER() over(Partition by cuisine order by no_of_orders desc) as rn from cte)a
where rn <=3

--2. Find Daily new customer count from the launch date (every day how many new customere are aquiring)

with cTTE as(
Select Customer_code, cast(Min(Placed_at) as date) as first_order_date  from orders
group by Customer_code
)
select first_order_date, count(*) as new_customer_perday 
from cTTE
group by first_order_date
order by first_order_date


--3. count of all the users whoe acquired in jan 2025 and only placed one order in jan and did not place any other order.
select * from orders
...
--4. list All thr customer with no order in the last 7 days but were acquired one month ago with their first order on promo.
with cte as(
select customer_code, min(Placed_at) as first_order_date, Max(Placed_at) as latest_order_date 
from orders
group by Customer_code)
select cte.*, Promo_code_Name 
from cte inner join orders o on cte.Customer_code = o.Customer_code and cte.first_order_date = o.Placed_at
where cte.latest_order_date <  DATEADD(DAY,-7,'2025-03-31') 
and first_order_date < DATEADD(month,-1,'2025-03-31') and Promo_code_Name is not null


--5. Growth team is planning to create a trigger that will target customers after their every
-- thired order with a personalized communication and they have asked you to create a query for this.
with cte as (
...
--6.List customer who placed more than 1 order and all their orders on a promo only 
Select Customer_code, count(*) as no_of_orders, count(Promo_code_Name) as Promo
from orders
group by Customer_code
having count(*) = count(Promo_code_Name) and count(*)> 1


--7. What percentage of customers were aquired were organically aquired in Jan 2025. (place their first order without promo code)
with CTE as(
Select *,
